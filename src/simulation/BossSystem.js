...
export const BossSystem = {
  spawnBoss(state, rng, bossId = null) {
    ...
    state.boss = boss;
    state.phase = "boss";
    state.events.push({ type: "boss-spawn", bossId: boss.id });
    return boss;
  },
  ...
  moveBoss(state, boss) {
    ...
    boss.x = clamp(boss.x + dir.x * (boss.speed / 60), boss.radius, ARENA_WIDTH - boss.radius);
    boss.y = clamp(boss.y + dir.y * (boss.speed / 60), boss.radius, ARENA_HEIGHT * 0.7);
  },
  ...
  runPattern(state, boss, rng) {
    ...
    if (boss.patternTick >= boss.patternDuration) {
      ...
      boss.patternDuration = patternDurationFor(boss.pattern, boss.phaseIndex);
    }
  }
};
