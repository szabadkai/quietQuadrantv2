...
export class BackgroundRenderer {
  constructor(scene) {
    ...
    this.layers = [
      ...
      { count: 24, speed: 0.25, size: [0.8, 1.5], alpha: 0.35, stars: [] },
  ...
  render(state) {
    ...
    const brightness =
      state?.phase === "intermission" ? 0.65 : state?.phase === "boss" ? 0.85 : 1;
  ...
  drawGrid(brightness) {
    const gridAlpha = 0.06 * brightness;
    this.graphics.lineStyle(1, 0x151e28, gridAlpha);
  ...
  drawStars(offsetX, offsetY, brightness) {
    for (const layer of this.layers) {
      ...
      for (const star of layer.stars) {
        ...
        this.graphics.fillStyle(0xffffff, star.alpha * brightness);
  ...
  drawBounds(brightness) {
    this.graphics.lineStyle(1, PALETTE_HEX.safe, 0.25 * brightness);
    this.graphics.strokeRect(1, 1, ARENA_WIDTH - 2, ARENA_HEIGHT - 2);
  }

  drawVignette() {
    this.graphics.fillStyle(0x000000, 0.18);
    this.graphics.fillRect(0, 0, ARENA_WIDTH, 24);
    this.graphics.fillRect(0, ARENA_HEIGHT - 24, ARENA_WIDTH, 24);
    this.graphics.fillRect(0, 0, 24, ARENA_HEIGHT);
    this.graphics.fillRect(ARENA_WIDTH - 24, 0, 24, ARENA_HEIGHT);
  }
}
