...
export class PlayerRenderer {
  ...
  render(players, interpolation) {
    ...
    for (const player of players) {
      ...
      if (ring) {
        const pulse = 0.45 + Math.sin(this.scene.time.now * 0.006) * 0.2;
        ring.clear();
      ...
      if (shieldRing) {
        const pulse = 0.35 + Math.sin(this.scene.time.now * 0.008) * 0.2;
  ...
  ensureNeutronRing(player) {
    const hasCore = player.neutronCore && (player.neutronBlockRadius ?? 0) > 0;
